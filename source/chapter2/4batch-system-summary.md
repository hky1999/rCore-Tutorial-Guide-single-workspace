...
批处理系统的所有应用加载到同一地址（`0x80400000`），采用"加载-执行-清空"模式。这简化了内存管理，但限制了并发性。
...
| 特性 | 第二章（批处理） | 第三章（多道程序） |
|-----|----------------|------------------|
| 程序加载地址 | 同一地址（`step = 0`） | 不同地址（`step = 2MB`） |
| 并发性 | 顺序执行 | 时分复用，协作式调度 |
| 内存占用 | 只保留当前程序 | 同时保留多个程序 |
| 任务切换 | 程序结束后加载下一个 | yield 系统调用主动让出 CPU |
| 复杂度 | 较低 | 增加任务管理和调度 |
...
在下一章中，我们将在批处理系统的基础上，引入任务切换和协作式调度，实现**多道程序系统**，进一步提升 CPU 利用率。
